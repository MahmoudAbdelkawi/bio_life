									...
									<div className="col-xl-4 col-md-6">
										<div className="blog-card mb-30">
											<div className="post-media">
												...
												// src={PF+blog.image}
												src={blogGridPic1} alt=""/></Link>
											</div>
											<div className="post-info">
												<ul className="post-meta">
													<li className="author"><Link to="/blog-details"><img 
													// src={PF+blog.author.image}
													src={testPic4} alt=""/> {blog.author}</Link></li>
													<li className="date"><i className="far fa-calendar-alt"></i> {new Date(blog.createdAt).toLocaleDateString()}</li>
												</ul>
												<h4 className="post-title"><Link to="/blog-details">{blog.title}</Link></h4>		
												<Link to={`/blog-details/${blog._id}`} className="btn btn-outline-primary btn-sm">Read More <i className="btn-icon-bx fas fa-chevron-right"></i></Link>		
											</div>
										</div>							
									</div>
								))}
